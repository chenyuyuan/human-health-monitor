...
public class SocketTask implements Runnable {
    ...
    private void handleSocket() throws Exception {
        ...
        MsgQueue.protocolState[taskNum-1] = 1;//修改协议状态为MODBUS，表明网关已经连接且使用MODBUS//added0526
        ...
        MsgQueue.sendMsgQueue.get(taskNum-1).offer(findSerialOrder);

        while (MsgQueue.sendMsgQueue.get(taskNum-1).size()==0) {//为空则线程休眠//modified0524
            Thread.sleep(1000);//1秒
        }
        //String orderString = sendMsgQueue[taskNum-1].poll();//取出一条命令//modified0524
        String orderString = MsgQueue.sendMsgQueue.get(taskNum-1).poll();//modified0524
        ...
        while (dis.read(bytes) != -1) {//如何实现循环接收的呢？忘了。。
            ...
            if (dis.available() == 0) { //客户端一条信息结束
                ...
                while (MsgQueue.sendMsgQueue.get(taskNum-1).isEmpty()) {//modified0524
                    Thread.sleep(1000);
                }
                orderString = MsgQueue.sendMsgQueue.get(taskNum-1).poll();//modified0524
